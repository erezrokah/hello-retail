...
const impl = {
  ...
  validateApiGatewayRequest: event => {
    if (!ajv.validate(twilioRequestSchemaId, event)) {
      ...
      return BbPromise.reject(
        new ClientError(
          `could not validate request to '${twilioRequestSchemaId}' schema. Errors: '${ajv.errorsText()}' found in event: '${JSON.stringify(
            event,
          )}'`,
        ),
      );
  ...
  validateTwilioRequest: event => {
    ...
    if (
      ...
      return BbPromise.reject(
        new AuthError('Twilio message signature validation failure!'),
      );
      ...
      return BbPromise.reject(
        new UserError(
          "Oops!  We can only accept standard images.  We weren't very creative...",
        ),
      );
    } else if (!body.From) {
      return BbPromise.reject(
        new ServerError(
          'Request from Twilio did not contain the phone number the image came from.',
        ),
      );
    } else {
      ...
      .then(
        data => {
          if (!data.Item) {
            return BbPromise.reject(
              new UserError(
                "Oops!  We couldn't find your assignment.  If you have registered and not completed your assignments, we will send one shortly.",
              ),
            );
          } else {
        ...
        ex =>
          BbPromise.reject(
            new ServerError(`Failed to retrieve assignment: ${ex}`),
          ), // eslint-disable-line comma-dangle
      );
  },
  /**
  ...
  storeImage: results => {
    ...
    return s3
      ...
      .then(
        ...
        ex =>
          BbPromise.reject(
            new ServerError(`Error placing image into S3: ${ex}`),
          ), // eslint-disable-line comma-dangle
      );
  },
  /**
  ...
  sendStepSuccess: results => {
    ...
    return stepfunctions
      ...
      .then(
        ...
        err =>
          BbPromise.reject(
            new ServerError(`Error sending success to Step Function: ${err}`),
          ), // eslint-disable-line comma-dangle
      );
  },
  userErrorResp: error => {
