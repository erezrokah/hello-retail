...
const impl = {
  ...
  queryAndAssignPhotographersByAssignmentCount: Promise.coroutine(function* qP(
    event,
    assignmentCount,
    priorData,
  ) {
    const queryParams = impl.queryPhotographersParams(
      assignmentCount,
      priorData,
    );
    ...
    if (data && data.Items && Array.isArray(data.Items) && data.Items.length) {
      // given a non-empty set of photographers, attempt assignment on the seemingly available ones
      for (let i = 0; i < data.Items.length; i++) {
        const item = data.Items[i];
        console.log(JSON.stringify(item, null, 2));
        if (
          // is the current photographer assignable?
          !item.assignment && // not assigned
          'assignments' in item &&
          Number.isInteger(item.assignments) && // valid assignments attribute
          'registrations' in item &&
          Number.isInteger(item.registrations) && // valid registrations attribute
      ...
      if (data.LastEvaluatedKey) {
        // if there are more photographers with the given assignment count, continue
        return impl.queryAndAssignPhotographersByAssignmentCount(
          event,
          assignmentCount,
          data,
        );
  ...
  assignPhotographers: Promise.coroutine(function* aP(event) {
    let photographer;
    for (let i = 0; i < 30; i++) {
      photographer = yield impl.queryAndAssignPhotographersByAssignmentCount(
        event,
        i,
      );
      console.log(
        `queryPhotographers[${i}] result: ${JSON.stringify(
          photographer,
          null,
          2,
        )}`,
