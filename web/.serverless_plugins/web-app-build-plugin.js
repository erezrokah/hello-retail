...
class ServerlessPlugin {
  constructor(serverless, options) {
    ...
    this.commands = {
      buildWebApp: {
        usage:
          'Consumes the serverless.yml template to construct the web application config.js containing the appropriate values.',
  ...
  getConfig() {
    return (
      ...
      this.serverless.service.custom.webAppBuild
    );
  }
  ...
  validateConfig(config) {
    if (!config) {
      ...
      this.serverless.cli.log(
        'Use `custom.webAppBuild` to define values needed.',
      );
      return false;
    }
    ...
    if (!config.configPath) {
      ...
      this.serverless.cli.log(
        "Use `custom.webAppBuild.configPath` to set the path to the write the web applicaiton's configuration file.",
      );
      return false;
    }
    ...
    if (!config.configValues) {
      ...
      this.serverless.cli.log(
        'Use `custom.webAppBuild.configValues` to define the set of config values to provide the web application.',
      );
      return false;
    }
    ...
    if (!config.webpackPath) {
      ...
      this.serverless.cli.log(
        'Use `custom.webAppBuild.webpackPath` to define the webpack config file to package the web app code.',
      );
      return false;
    }
  ...
  buildWebApp() {
    ...
    this.serverless.cli.log(
      `Running webpack to build application with config ${
        config.webpackPath
      } ...`,
    );

    const process = spawnSync(`${__dirname}/../../node_modules/.bin/webpack`, [
      '--config',
      config.webpackPath,
    ]);
  ...
  uploadWebApp() {
    ...
    this.serverless.cli.log(
      `Uploading files from /app to S3 bucket ${this.getBucketName()}...`,
    );
    ...
    this.serverless.cli.log(
      `Upload to S3 bucket ${this.getBucketName()} Done.`,
    );
  }
  ...
  emptyWebAppBucket() {
    ...
    this.serverless.cli.log(
      `Emptying WebApp S3 bucket ${this.getBucketName()} ...`,
    );
  ...
  getImportValue(name, outputName) {
    return new Promise((resolve, reject) => {
      ...
      if (importParts.length < 3) {
        reject(
          `Expected Fn::Import to match pattern: 'service:stage:outputName', but got '${outputName}' instead.`,
        );
      }
  ...
  configureWebApp() {
    ...
    for (const name of Object.keys(config.configValues)) {
      ...
      if (typeof value === 'string') {
        ...
        this.serverless.cli.log(
          `WARNING: Web App config property ${name} was not a string. Ignoring.`,
        );
      }
    }
    ...
    pendingImports.forEach(imported => {
      importPromises.push(
        this.getImportValue(imported.name, imported.outputName),
      );
    });

    return Promise.all(importPromises).then(outputs => {
      ...
      this.serverless.cli.log(
        `Writing web config file to ${config.configPath} ...`,
      );
