...
class NewProductPage extends Component {
  ...
  validateProduct(property, value) {
    ...
    this.setState({
      ...
      isProductValid:
        product.category &&
        product.category.match(/^[\w\d]+/) &&
        product.name &&
        product.name.match(/^[\w\d]+/) &&
        product.brand &&
        product.brand.match(/^[\w\d]+/) &&
        product.description &&
        product.description.match(/^[\w\d]+/),
    });
  }
  ...
  createProduct() {
    ...
    this.props.awsLogin
      .makeApiRequest(config.EventWriterApi, 'POST', '/event-writer/', {
        ...
        id: `0000000${Math.floor(Math.abs(Math.random() * 10000000))}`.substr(
          -7,
        ),
        origin: `hello-retail/web-client-create-product/${
  ...
  render() {
    ...
    return (
      <div>
        ...
        <button
          disabled={!this.state.isProductValid}
