...
class AmazonLogin extends Component {
  ...
  constructor(props) {
    ...
    this.assumeWebAppIdentityWithToken = this.assumeWebAppIdentityWithToken.bind(
      this,
    );
    ...
    this.performLoginAndAssumeIdentity = this.performLoginAndAssumeIdentity.bind(
      this,
    );
  ...
  sendUserLogin() {
    this.makeApiRequest(config.EventWriterApi, 'POST', '/event-writer/', {
      ...
      origin: `hello-retail/web-client-login-user/${this.state.profile.id}/${
  ...
  makeApiRequest(api, verb, path, data) {
    return new Promise((resolve, reject) => {
      ...
      signer.addAuthorization(
        this.webApplicationIdentityCredentials,
        new Date(),
      );
