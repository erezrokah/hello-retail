...
class ProductDetailPage extends Component {
  ...
  productsLoaded(products) {
    ...
    this.setState({
      ...
      id: p.id,
      image: p.image ? `https://s3.amazonaws.com/${p.image}` : null,
    });
  }
  ...
  purchaseProduct() {
    this.props.awsLogin
      .makeApiRequest(config.EventWriterApi, 'POST', '/event-writer/', {
        ...
        origin: `hello-retail/web-client-purchase-product/${
  ...
  addToCart() {
    this.props.awsLogin
      .makeApiRequest(config.EventWriterApi, 'POST', '/event-writer/', {
        ...
        id: this.props.params.id,
        origin: `hello-retail/web-client-cart-product/${
          this.props.awsLogin.state.profile.id
        }/${this.props.awsLogin.state.profile.name}`,
      })
      .then(() => {
    ...
    return (
      <div>
        <div>
          ...
          />
          <button style={backButtonStyle} onClick={browserHistory.goBack}>
            Back to List
          </button>
        </div>
      </div>
