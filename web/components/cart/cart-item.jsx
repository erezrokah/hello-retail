...
class CartItem extends Component {
  ...
  constructor(props) {
    ...
    this.getProductByIdFromDynamoAsync = this.getProductByIdFromDynamoAsync.bind(
      this,
    );
  ...
  componentDidMount() {
    ...
    if (this.props.productId) {
      return this.getProductsByIdAsync(this.props.productId).then(
        this.productsLoaded,
      );
    } else {
  ...
  getProductByIdFromDynamoAsync(productId) {
    const params = {
      AttributesToGet: ['brand', 'description', 'name', 'id', 'image'],
  ...
  getProductsByIdAsync(productId) {
    return this.getProductByIdFromDynamoAsync(productId).then(data => {
      const productList = [];
      productList.push({
  ...
  removeFromCart() {
    this.props.awsLogin
      .makeApiRequest(config.EventWriterApi, 'POST', '/event-writer/', {
        ...
        origin: `hello-retail/web-client-cart-remove/${
          this.props.awsLogin.state.profile.id
        }/${this.props.awsLogin.state.profile.name}`,
      })
      .then(() => {
        this.setState({
