...
const impl = {
  ...
  clientError: (schemaId, ajvErrors, event) =>
    impl.response(
      400,
      `${constants.METHOD_CATEGORIES} ${
        constants.INVALID_REQUEST
      } could not validate request to '${schemaId}' schema. Errors: '${ajvErrors}' found in event: '${JSON.stringify(
        event,
  ...
  dynamoError: err => {
    ...
    return impl.response(
      500,
      `${constants.METHOD_CATEGORIES} - ${constants.INTEGRATION_ERROR}`,
    );
  },
...
const api = {
  ...
  categories: (event, context, callback) => {
    if (!ajv.validate(categoryRequestSchemaId, event)) {
      ...
      callback(
        null,
        impl.clientError(categoryRequestSchemaId, ajv.errorsText()),
        event,
      );
    } else {
      const params = {
      ...
      dynamo.scan(params, (err, data) => {
        if (err) {
          ...
          callback(
            null,
            impl.securityRisk(categoryItemsSchemaId, ajv.errorsText()),
            data.Items,
          ); // careful if the data is sensitive
        } else {
          // valid
  ...
  products: (event, context, callback) => {
    if (!ajv.validate(productsRequestSchemaId, event)) {
      ...
      callback(
        null,
        impl.clientError(productsRequestSchemaId, ajv.errorsText(), event),
      );
    } else {
      const params = {
      ...
      dynamo.query(params, (err, data) => {
        if (err) {
          ...
          callback(
            null,
            impl.securityRisk(productItemsSchemaId, ajv.errorsText()),
            data.Items,
          ); // careful if the data is sensitive
        } else {
          // valid
